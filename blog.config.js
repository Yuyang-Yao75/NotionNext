...
const DEDICATED_NOTION_API_BASE_URL = 'https://yyyao.notion.site/api/v3'
const configuredNotionApiBaseUrl = String(
  process.env.API_BASE_URL || ''
).replace(/\/+$/, '')
const sharedNotionApiBaseUrls = new Set([
  'https://app.notion.com/api/v3',
  'https://www.notion.so/api/v3'
])
const notionApiBaseUrl =
  configuredNotionApiBaseUrl &&
  !sharedNotionApiBaseUrls.has(configuredNotionApiBaseUrl)
    ? configuredNotionApiBaseUrl
    : DEDICATED_NOTION_API_BASE_URL

const BLOG = {
  API_BASE_URL: notionApiBaseUrl, // 共享 API 域名异常时使用本站专属 Notion 域名；其它自定义代理仍被保留
