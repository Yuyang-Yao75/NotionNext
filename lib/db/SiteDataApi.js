  ...
  const data = await getOrSetDataWithCache(cacheKey,
    async () => {
      ...
      const originalPageRecordMap = await fetchNotionPageBlocks(pageId, from, {
        validate: recordMap => assertCompleteSiteRecordMap(recordMap, pageId)
      })
      // 转换格式
      const r = await convertNotionToSiteData(pageId, from, originalPageRecordMap)
      assertUsableSiteData(r, pageId)
      // 返回并塞入缓存
...
function assertUsableSiteData(data, pageId) {
  const minimumPosts = Math.max(0, Number(BLOG.SITE_DATA_MIN_POSTS) || 0)
  const postCount = Number(data?.postCount) || 0
  const configCount = Object.keys(data?.NOTION_CONFIG || {}).length

  if (!Array.isArray(data?.allPages) || postCount < minimumPosts) {
    const error = new Error(
      `Notion site ${pageId} produced ${postCount} published posts; expected at least ${minimumPosts}`
    )
    error.code = 'NOTION_INCOMPLETE_SITE_DATA'
    throw error
  }

  if (BLOG.SITE_DATA_REQUIRE_CONFIG && configCount === 0) {
    const error = new Error(
      `Notion site ${pageId} produced no enabled site configuration`
    )
    error.code = 'NOTION_INCOMPLETE_SITE_DATA'
    throw error
  }
...
function handleDataBeforeReturn(db) {
  ...
  if (db.NOTION_CONFIG.LINK) {
    db.NOTION_CONFIG.LINK = normalizeSiteUrl(db.NOTION_CONFIG.LINK)
  }
  if (db.siteInfo?.link) {
