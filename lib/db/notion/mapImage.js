...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  const notionHost = BLOG.NOTION_HOST.replace(/\/$/, '')
  let ret = null
  // 相对目录，则视为notion的自带图片
  if (img.startsWith('/')) {
    ret = notionHost + img
  ...
  // Never mutate a signed URL. Extra query parameters invalidate its
  // canonical request and make uploaded page icons fail to load.
  if (
    ret.startsWith('data:') ||
    ret.startsWith('blob:') ||
    isSignedNotionFileUrl(ret) ||
    isNotionUserContentUrl(ret)
  ) {
    return ret
  }

  const hasConverted =
    ret.indexOf(`${notionHost}/image`) === 0 ||
  ...
  if (needConvert) {
    ret =
      ...
      '?table=' +
      getNotionImageTable(block, type) +
      '&id=' +
      (block?.id || '')
  }
...
/**
 * Notion upload URLs are query-signed. Changing their query string breaks the
 * signature, so they must pass through the renderer unchanged.
 */
function isSignedNotionFileUrl(image) {
  try {
    const url = new URL(image)
    return (
      (url.hostname === 'file.notion.so' &&
        url.searchParams.has('expirationTimestamp')) ||
...
function isNotionUserContentUrl(image) {
  try {
    return new URL(image).hostname === 'img.notionusercontent.com'
  } catch {
    return false
  }
}

function getNotionImageTable(block, fallback = 'block') {
  let table = block?.parent_table || fallback
  if (table === 'space' || table === 'collection' || table === 'team') {
    table = 'block'
  }
  return table || 'block'
}

export { compressImage, getNotionImageTable, isSignedNotionFileUrl, mapImgUrl }
