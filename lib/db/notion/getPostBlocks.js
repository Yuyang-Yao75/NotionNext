...
function getRecordMapBlock(recordMap, id) {
  const normalizedId = normalizeNotionId(id)
  const entry = Object.entries(recordMap?.block || {}).find(
    ([blockId, blockEntry]) =>
      normalizeNotionId(blockId) === normalizedId ||
      normalizeNotionId(getBlockValue(blockEntry)?.id) === normalizedId
  )
  return getBlockValue(entry?.[1])
}

function getRecordMapValueById(table, id) {
  const normalizedId = normalizeNotionId(id)
  const entry = Object.entries(table || {}).find(([entryId, entryValue]) => {
    const value = getBlockValue(entryValue)
    return (
      normalizeNotionId(entryId) === normalizedId ||
      normalizeNotionId(value?.id) === normalizedId
    )
  })
  return getBlockValue(entry?.[1])
}
...
/**
 * Public search results are not guaranteed to use the collection view's row
 * order. Menu/SubMenu grouping relies on that order, so keep recovered rows in
 * the primary view's manual page_sort order and append genuinely new rows in
 * the stable order returned by search.
 */
function orderRowsByPrimaryView(rowIds, recordMap, searchRecordMap, viewId) {
  const primaryView =
    getRecordMapValueById(recordMap?.collection_view, viewId) ||
    getRecordMapValueById(searchRecordMap?.collection_view, viewId)
  const pageSort = primaryView?.page_sort
  if (!Array.isArray(pageSort) || pageSort.length === 0) return rowIds

  const recoveredRows = new Map(
    rowIds.map(rowId => [normalizeNotionId(rowId), rowId])
  )
  const orderedRows = []

  pageSort.forEach(rowId => {
    const normalizedId = normalizeNotionId(rowId)
    if (!recoveredRows.has(normalizedId)) return
    orderedRows.push(recoveredRows.get(normalizedId))
    recoveredRows.delete(normalizedId)
  })

  return [...orderedRows, ...recoveredRows.values()]
}

/**
 * Notion's queryCollection endpoint can return an empty primary view from
 * Vercel while the public search endpoint still exposes every database row.
 * Rebuild only the primary view query from direct children of the collection;
 * regular collection filtering still happens later in SiteDataApi.
 */
export async function recoverPrimaryCollectionRows(recordMap, pageId) {
  const rootBlock = getRecordMapBlock(recordMap, pageId)
  const collectionId = rootBlock?.collection_id
  const primaryViewId = rootBlock?.view_ids?.[0]
  if (!collectionId || !primaryViewId || typeof notionAPI.search !== 'function') {
    return recordMap
  }

  const primaryQuery = recordMap?.collection_query?.[collectionId]?.[primaryViewId]
  const currentIds = primaryQuery?.collection_group_results?.blockIds || []
  if (currentIds.length > 0) return recordMap

  const searchResult = await notionAPI.search({
    ancestorId: collectionId,
    query: '',
    limit: 100
  })
  const searchRecordMap = searchResult?.recordMap || {}
  const recoveredRowIds = Object.entries(searchRecordMap.block || {})
    .filter(([, entry]) => {
      const block = getBlockValue(entry)
      return (
        block?.type === 'page' &&
        block?.parent_table === 'collection' &&
        normalizeNotionId(block?.parent_id) === normalizeNotionId(collectionId)
      )
    })
    .map(([blockId, entry]) => getBlockValue(entry)?.id || blockId)

  const rowIds = orderRowsByPrimaryView(
    recoveredRowIds,
    recordMap,
    searchRecordMap,
    primaryViewId
  ...
  if (rowIds.length === 0) return recordMap

  recordMap.block = { ...recordMap.block, ...searchRecordMap.block }
  recordMap.collection = {
    ...recordMap.collection,
    ...searchRecordMap.collection
  }
  recordMap.collection_view = {
    ...recordMap.collection_view,
    ...searchRecordMap.collection_view
  }
  recordMap.notion_user = {
    ...recordMap.notion_user,
    ...searchRecordMap.notion_user
  }
  ...
  recordMap.collection_query[collectionId] = {
    ...
    [primaryViewId]: {
      ...primaryQuery,
      collection_group_results: { blockIds: rowIds }
    }
  }

  console.log(
    '[getPage] recovered primary collection rows from public search:',
    rowIds.length
  )
  return recordMap
}
 ...
 * @param {{ cacheVersion?: string | number | Date, validate?: (recordMap: object) => void }} options
 */
export async function fetchNotionPageBlocks(id, from = undefined, options = {}) {
  ...
  let pageBlock = await getOrSetDataWithCache(cacheKey, loadPage)

  if (pageBlock && validate) {
    try {
      validate(pageBlock)
    } catch (error) {
      console.warn('[getPage] invalid cached pageBlock, refreshing:', id, error)
      await delCacheData(cacheKey)
      pageBlock = await loadPage()
      if (pageBlock) await setDataToCache(cacheKey, pageBlock, null)
    }
  }
...
export async function getPageWithRetry(
  id,
  from,
  retryAttempts = PAGE_FETCH_RETRY_ATTEMPTS,
  ...
  try {
    ...
    const pageData = await notionAPI.getPage(
      id,
      validate
        ? { throwOnCollectionErrors: true, concurrency: 1 }
        : undefined
    )
    if (validate) {
      try {
        ...
        if (
          validationError?.code === 'NOTION_INCOMPLETE_DATABASE_RESPONSE'
        ) {
          await recoverPrimaryCollectionRows(pageData, id)
        }
        validate(pageData)
      }
    }
    ...
    if (pageBlock) {
      try {
        if (validate) validate(pageBlock)
        return pageBlock
      } catch (cacheError) {
        console.warn('[getPage] cached fallback is incomplete:', cacheError)
        await delCacheData(cacheKey)
      }
    }

    if (retryAttempts <= 1) {
      console.error('[getPage] request failed after retries:', `from:${from}`, `id:${id}`)
      throw e
    }

    const retryDelayMs = getNotionRetryDelayMs(attemptNumber)
    console.warn(
      '[getPage] retrying after transient failure:',
      `id:${id}`,
...
export function getNotionRetryDelayMs(attemptNumber) {
