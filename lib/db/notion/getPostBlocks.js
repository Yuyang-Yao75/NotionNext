...
export async function fetchNotionPageBlocks(
  ...
  await refreshPageIconUrls(pageBlock, cacheKey)
...
/**
 * notion-client signs page covers and media, but not page icons belonging to
 * rows in an inline database. Sign those icons separately and keep their
 * original source so a cached record can be refreshed after expiry.
 */
export async function refreshPageIconUrls(
  recordMap,
  cacheKey,
  bufferMs = 10 * 60 * 1000
) {
  const files = getPageIconFileInstances(recordMap, bufferMs)
  if (!files.length) return false

  try {
    const { signedUrls } = await notionAPI.getSignedFileUrls(
      files.map(file => ({
        permissionRecord: file.permissionRecord,
        url: file.url
      }))
    )
    if (!signedUrls?.length) return false

    let changed = false
    files.forEach(({ block, source }, index) => {
      const signedUrl = signedUrls[index]
      if (!signedUrl) return

      block.format.page_icon_source = source
      block.format.page_icon = signedUrl
      changed = true
    })
    ...
    console.warn('[Notion page icons] refresh failed:', err)
    return false
  }
}

function getPageIconFileInstances(recordMap, bufferMs) {
  return Object.values(recordMap?.block || {}).flatMap(entry => {
    const block = getBlockValue(entry)
    const currentIcon = block?.format?.page_icon
    const source = block?.format?.page_icon_source || currentIcon

    if (
      !block?.id ||
      !source ||
      !isRefreshableNotionFileSource(source) ||
      !shouldRefreshPageIcon(currentIcon, bufferMs)
    ) {
      return []
    }
...
function shouldRefreshPageIcon(icon, bufferMs) {
  if (!icon || icon.startsWith('attachment:')) return true

  const expiresAt = getSignedUrlExpiration(icon)
  if (Number.isFinite(expiresAt)) {
    return expiresAt <= Date.now() + bufferMs
  }

  return isNotionUploadedFileUrl(icon)
}

function getSignedUrlExpiration(url) {
  try {
    const parsed = new URL(url)
    const notionExpiration = Number(
      parsed.searchParams.get('expirationTimestamp')
    )
    if (Number.isFinite(notionExpiration) && notionExpiration > 0) {
      return notionExpiration < 1e12
        ? notionExpiration * 1000
        : notionExpiration
    }

    const amzDate = parsed.searchParams.get('X-Amz-Date')
    const amzExpires = Number(parsed.searchParams.get('X-Amz-Expires'))
    if (/^\d{8}T\d{6}Z$/.test(amzDate || '') && Number.isFinite(amzExpires)) {
      ...
      return signedAt + amzExpires * 1000
    }
  } catch {
    return Number.NaN
  }
...
function isRefreshableNotionFileSource(source) {
  return source.startsWith('attachment:') || isNotionUploadedFileUrl(source)
}

function isNotionUploadedFileUrl(source) {
  if (isNotionHostedFileUrl(source)) return true
